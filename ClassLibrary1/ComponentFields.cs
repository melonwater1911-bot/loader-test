...
partial class NobndlLoaderPlugin
{
    ...
    private void ReportMaterials(Component target,NobndlFieldRecord record,object value)
    {
        ...
        for (int i=0; i<applied.Length; i++)
        {
            ...
            text.Append(" -> ");
            text.Append(applied[i]!=null ? applied[i].name+" ["+NobndlMaterialFactory.ShaderName(applied[i].shader)+"]" : "NULL");
        }
    ...
    private object PrepareIfMaterial(object value)
    {
        ...
        if (material!=null)
            return materialFactory.Prepare(cache,material);
        Material[] array=value as Material[];
        if (array!=null)
            return materialFactory.Prepare(cache,array);
        return value;
    }
