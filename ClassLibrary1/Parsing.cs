...
partial class Loader
{
    ...
    private static AnimationCurve ParseAnimationCurve(string value)
    {
        ...
        if(string.IsNullOrEmpty(value)) return curve;
        string[] parts=value.Split('|');
        if(parts.Length<3) return curve;
        try
        {
            ...
            for (int i=0; i<keyCount; i++)
            {
                ...
                var key = new Keyframe(ParseFloat(k[0]),ParseFloat(k[1]),ParseFloat(k[2]),ParseFloat(k[3]));
    ...
    {
        string[] parts=(value ?? "").Split(',');
        if(parts.Length<count) Array.Resize(ref parts,count);
